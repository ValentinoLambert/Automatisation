...
Pour initialiser le projet avec les données, exécutez les 2 commandes suivantes :

```bash
export DATABASE_URL="sqlite:///%kernel.project_dir%/var/data.db" && export MESSENGER_TRANSPORT_DSN=doctrine://default && php bin/console doctrine:migrations:migrate --no-interaction
```

```bash
export DATABASE_URL="sqlite:///%kernel.project_dir%/var/data.db" && export MESSENGER_TRANSPORT_DSN=doctrine://default && php bin/console doctrine:fixtures:load --no-interaction
```
